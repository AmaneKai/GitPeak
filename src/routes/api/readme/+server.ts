...
interface GithubStats {
  ...
  totalRepos: number
  languages: Language[]
  mostStarredRepo: TopRepo | null
}

interface DonutSlice extends Language {
  color: string
  startDeg: number
  endDeg: number
}
...
function donutArcPath(cx: number, cy: number, ro: number, ri: number, s: number, e: number) {
  ...
  return [
    `M ${o1.x} ${o1.y}`,
    `A ${ro} ${ro} 0 ${la} 1 ${o2.x} ${o2.y}`,
    `L ${i1.x} ${i1.y}`,
    `A ${ri} ${ri} 0 ${la} 0 ${i2.x} ${i2.y} Z`
  ].join(' ')
}
    ...
    <defs>
      <style>
        @font-face {
          ...
          src: url(data:font/truetype;charset=utf-8;base64,${mono}) format('truetype');
        }
        @font-face {
          font-family: 'Instrument Serif';
          src: url(data:font/truetype;charset=utf-8;base64,${serif}) format('truetype');
        }
        ...
        .text-subtle {
          font-family: 'JetBrains Mono', monospace; fill: #908caa; font-size: 11px;
        ...
        .slice {
          transform-origin: 728px 170px; opacity: 0;
          animation: scaleIn 0.8s cubic-bezier(0.16, 1, 0.3, 1) forwards;
        }
        .bar-fill {
          transform-origin: left center;
          animation: stretch 1.2s cubic-bezier(0.16, 1, 0.3, 1) forwards;
        }
      </style>
...
function svgProfile(stats: GithubStats, username: string, avatar: string) {
  ...
  return `
    <g class="stagger delay-1">
      <image href="data:image/png;base64,${avatar}" x="40" y="40" width="80" height="80"
          clip-path="url(#av-clip)" filter="url(#shadow)" />
      <rect x="40" y="40" width="80" height="80" rx="20" fill="none" stroke="#c4a7e7"
...
function statCardSvg(id: string, x: number, y: number, accent: string, label: string, v: string) {
  return `
    <clipPath id="${id}">
      <rect x="${x}" y="${y}" width="224" height="116" rx="16" />
    </clipPath>
    <rect x="${x}" y="${y}" width="224" height="116" rx="16" fill="#1f1d2e"
        fill-opacity="0.6" stroke="rgba(144,140,170,0.15)" filter="url(#shadow)" />
...
function svgStatGrid(stats: GithubStats) {
  const a = formatNumber(stats.totalContributions)
  const b = formatNumber(stats.totalCommits)
  const c = formatNumber(stats.totalStars)
  const d = formatNumber(stats.totalRepos)
  const accent = COLORS.foam

  return `
    <g class="stagger delay-2">
      ${statCardSvg('sc1', 40, 156, COLORS.foam, 'Contributions', a)}
      ${statCardSvg('sc2', 280, 156, COLORS.foam, 'Commits', b)}
      ${statCardSvg('sc3', 40, 288, COLORS.foam, 'Stars', c)}
      ${statCardSvg('sc4', 280, 288, COLORS.foam, 'Repositories', d)}
    </g>`
}

function svgTopRepo(repo: TopRepo | null) {
  if (!repo) return ''
  const val = formatNumber(repo.stars)
  return `
    <g class="stagger delay-3">
      <rect x="40" y="420" width="464" height="72" fill="#1f1d2e" fill-opacity="0.6"
          stroke="rgba(144,140,170,0.15)" rx="16" filter="url(#shadow)" />
      ...
      <text x="480" y="474" class="text-main" fill="#f6c177" font-weight="bold"
          font-size="20" text-anchor="end">&#9733; ${val}</text>
    </g>`
}

function svgLangPanel(slices: DonutSlice[], total: number, avatar: string) {
  const paths = slices.map((s, i) => {
    const d = donutArcPath(728, 170, 75, 57, s.startDeg, s.endDeg)
    const delay = 0.4 + i * 0.08
    return `<path d="${d}" fill="${s.color}" class="slice" style="animation-delay:${delay}s"/>`
  }).join('')

  const bars = slices.slice(0, 5).map((s, i) => {
    const y = 300 + i * 30
    const delay = 0.6 + i * 0.1
    const w = (s.percentage / 100) * 140
    return `
      <circle cx="572" cy="${y - 4}" r="4" fill="${s.color}" />
      <text x="588" y="${y}" class="text-main" font-size="12">${s.name}</text>
      <rect x="700" y="${y - 6}" width="140" height="4" fill="rgba(144,140,170,0.1)" rx="2" />
      <rect x="700" y="${y - 6}" width="${w}" height="4" fill="${s.color}" rx="2"
          class="bar-fill" style="animation-delay:${delay}s" filter="url(#glow)" />
      <text x="890" y="${y}" class="text-main" font-size="12" font-weight="600"
          fill="${s.color}" text-anchor="end">${s.percentage}%</text>`
  }).join('')

  return `
    <g class="stagger delay-4">
      <clipPath id="lang-clip">
        <rect x="536" y="40" width="384" height="452" rx="24" />
      </clipPath>
      <rect x="536" y="40" width="384" height="452" rx="24" fill="#1f1d2e"
          fill-opacity="0.6" stroke="rgba(144,140,170,0.15)" filter="url(#shadow)" />
      <rect x="536" y="40" width="384" height="4" fill="#c4a7e7" clip-path="url(#lang-clip)" />
      
      <text x="568" y="82" class="text-subtle">Languages</text>
      <rect x="830" y="65" width="60" height="20" rx="10" fill="rgba(196,167,231,0.1)"
          stroke="rgba(196,167,231,0.2)" />
      <text x="860" y="79" class="text-main" fill="#c4a7e7" font-size="10"
          text-anchor="middle">${total} used</text>
      <circle cx="728" cy="170" r="66" fill="none" stroke="rgba(144,140,170,0.05)"
          stroke-width="18" />
      ${paths}
      <image href="data:image/png;base64,${avatar}" x="680" y="122" width="96" height="96"
          clip-path="url(#do-clip)" />
...
function buildSvgTemplate(
  ...
  return `
    <svg width="960" height="520" viewBox="0 0 960 520" fill="none"
      ...
      ${svgProfile(stats, user, av)}
      ${svgStatGrid(stats)}
      ${svgTopRepo(stats.mostStarredRepo)}
      ${svgLangPanel(slices, stats.languages.length, av)}
    </svg>`
}
...
export async function GET({ url }: { url: URL }) {
  ...
  const [monoReg, serifReg, avatarB64] = await Promise.all([
    loadFontB64('JetBrains Mono', 400),
    loadFontB64('Instrument Serif', 400),
    fetchBase64(stats.avatarUrl)
  ])

  const slices = buildDonutSlices(stats.languages)
  const svg = buildSvgTemplate(stats, username, slices, monoReg, serifReg, avatarB64)
