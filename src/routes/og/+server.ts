...
// ==========================================
// 1. Font Loader (Forces TTF fallback)
// ==========================================
...
// ==========================================
// 2. Pie Chart Math (Recreated for Satori)
// ==========================================
...
export async function GET({ url }) {
	...
	const statsRes = await fetchStats(username);
	if (!statsRes.ok) return new Response('User not found', { status: 404 });
	const stats = statsRes.data;

	// Fetch fonts concurrently (JetBrains Mono + Instrument Serif + Noto Sans for Asian chars)
	...
	// Construct exactly 1 raw string to avoid Satori HTML parser bugs
	// Note: EVERY single div must have "display: flex" - Satori enforces this strictly!
	const template = `
