<script lang="ts">
  ...
  const dotMatrix = $derived.by(() => {
    ...
    let remIndex = 0
    while (currentTotal < totalDots && remIndex < remainders.length) {
      counts[remainders[remIndex].index].dotCount++
      currentTotal++
      remIndex++
    }
    ...
    const matrix: string[] = []
    counts.forEach((c) => {
      for (let index = 0; index < c.dotCount; index++) matrix.push(c.color)
    })
...
<div
  ...
  "
>
  {#each dotMatrix as color, index (index)}
    <div
      class="matrix-dot"
      style="
        width: {dotSize}px; 
